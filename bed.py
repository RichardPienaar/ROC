'''
Author -> Mikael Boden 
 - Some small additions to BedFile class - Richard
'''
import ival
...
def dist_promoter_or_enchancer(entry1, entry2, signed = False):
    """ Calculate and return the BedEntry with the closest distance (from MIDDLE of the upper interval to the end of the closest interval downstream).
        If signed is True, the distance is negative if this interval is after the that.
    """
    if isinstance(entry1, BedEntry) and isinstance(entry2, BedEntry):
        if (entry1.chrom == entry2.chrom):
            return ival.distance_promoter_or_enhancer(entry1.getInterval(), entry2.getInterval(), signed)
    return None
...
class BedFile:
    ...
    def __init__(self, entries, format = 'Limited',chr='chr1'):
        """
        ...
        if isinstance(entries, str): # filename
            if format == 'BedGraph':
                self.chroms = readBedGraphFile(entries,chr)
...
def readBedFile(filename, format = 'Limited'):
    ...
    for line in f:
        ...
        try:
            ...
            elif format.lower() == 'idr': # idr - ADDED Richard
                entry.addOption(name = words[3], score = float(words[6]), pValue = float(words[7]))
