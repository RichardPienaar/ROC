...
class Interval:
    ...
    def dist_promoter_or_enhancer(self, that, signed = False):
        '''
        same as dist but for centre of the first --> upper boundary of the second
        '''
        thiscentre = (self.max - self.min) / 2 + self.min
        thatcentre = (that.max - that.min) / 2 + that.min

        if (self.min > that.max): return thiscentre - that.max
        if (self.max < that.min): return thatcentre - self.max
...
def distance_promoter_or_enhancer(first, second, signed = False):
    """ Calculate and return the closest distance (from one end of the interval to the other).
        Returns distance = centre of the first interval to the boundary of the second
        If signed is True, the distance is negative if the first is after the second.
    """
    if isinstance(first, Interval) and isinstance(second, Interval):
        return first.dist_promoter_or_enhancer(second, signed)
    return RuntimeError
